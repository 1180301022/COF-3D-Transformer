...
from tr_spe import Embeddings, FeedForward, Generator3D
from tr_afps import MultiRelationEncoder, EncoderLayer
from tr_msa import MultiScaleMultiHeadedAttention
from tr_spe import MultiHeadedAttention
from tr_spe import SinusoidalPositionEncoding3D


def build_model(vocab, tgt, dist_bar, k, N=6, embed_dim=512, ffn_dim=2048, head=8, dropout=0.1):
    assert k > 1
    c = copy.deepcopy
    attn = MultiScaleMultiHeadedAttention(head, embed_dim, dist_bar)  # MSA
...
class FullTransformer3D(nn.Module):
    ...
    def forward(self, src, src_mask, dist):
        ...
        encode = self.encoder(embed, dist, src_mask) #[:, 0]
