...
def attention(query, key, value, dist_conv, mask=None, dropout=None):
    ...
    scores *= dist_conv  # scores是Transformer中的A，未经过softmax，dist维度 B，H，N，N，是卷积后的结果
    # 得到的scores即为 Aconv

    if mask is not None:
        scores = scores.masked_fill(mask == 0, -1e9)
    p_attn = F.softmax(scores, dim=-1)  # 对Aconv进行softmax，得到处理后的注意力矩阵
    if dropout is not None:
        p_attn = dropout(p_attn)
    return torch.matmul(p_attn, value), p_attn  # 输出加权和z，注意力矩阵 0~1
...
class MultiHeadedAttention(nn.Module):
    ...
    def forward(self, query, key, value, dist_conv, mask=None):
        ...
        dist_conv = self.cnn(dist_conv.unsqueeze(1))  # 卷积前（距离矩阵）1，N，N -> B，1，N，N ->卷积后B，H，N，N
        ...
        return self.linears[-1](x)  # 小分子的位置嵌入完成
