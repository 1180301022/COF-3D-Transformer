...
class MultiScaleTransformer3D(nn.Module):
    ...
    def forward(self, src, src_mask, dist):
        x = self.src_embed(src)  # vocab -> embed_dim
        if self.feat_embed: x += self.feat_embed(src)  # 性质嵌入
...
def attention(query, key, value, dist_conv, dist, dist_bar, mask=None, dropout=None):
    ...
    if mask is not None:
        scores = scores.masked_fill(mask == False, -1e10)
    ...
    for i in dist_bar:
        ...
        out.append(torch.matmul(p_attn, value))  # (h, h) * (h, d_k) = (h, d_k) = embed_dim
    return out, p_attn
...
class MultiScaleMultiHeadedAttention(nn.Module):
    ...
    def forward(self, query, key, value, dist, mask=None):
        ...
        return self.scale_linear(x), self.attn
