...
#######################################
## 只有encoder部分的Transformer
#######################################

def build_model(vocab, tgt, N=6, embed_dim=512, ffn_dim=2048, head=8, dropout=0.1):
    c = copy.deepcopy
    attn = MultiHeadedAttention(head, embed_dim)  # 多头注意力模块
    ff = FeedForward(embed_dim, ffn_dim, dropout)   # 简单的前馈层
    position = SinusoidalPositionEncoding3D(embed_dim, dropout)  # 大分子位置嵌入
...
class Transformer3D(nn.Module):
    ...
    def forward(self, src, src_mask, pos):
        a = self.encoder(self.src_pe(self.src_embed(src), pos), src_mask)
        attn_out = a[:, 0]
        return self.generator(attn_out)
...
class LayerNorm(nn.Module):
    ...
    def forward(self, x):
        ...
        std = x.std(-1, keepdim=True) + self.eps  # 计算标准差，keepdim保持维度和原来一样，有可能某一维变成1
        return self.a_2 * (x - mean) / std + self.b_2

# class LayerNorm(nn.LayerNorm):
#     def __init__(self, size, eps=1e-6, b=True):
#         super(LayerNorm, self).__init__(size, eps, b)
...
class MultiHeadedAttention(nn.Module):
    def __init__(self, h, embed_dim, dropout=0.1):
        ...
        assert embed_dim % h == 0  # 因为要把 embed_dim 分成h份，提取多种语义
        self.h = h
...
class SinusoidalPositionEncoding3D(nn.Module):
    ...
    def forward(self, x, pos):
        ...
        # 计算x,y,z轴的位置嵌入结果，和输入相加
        div = torch.exp(torch.arange(0., self.embed_dim, 2) * -(math.log(10000.0) / self.embed_dim)).float().cuda(0)
        for i in range(3):
            pe = torch.zeros(x.shape).cuda(0)
            pe[..., 0::2] = torch.sin(pos[..., i].unsqueeze(2) * div)
            pe[..., 1::2] = torch.cos(pos[..., i].unsqueeze(2) * div)
            x += Variable(pe, requires_grad=False)
        return self.dropout(x)  # 返回大分子位置嵌入后的结果
